...
from cross_db_benchmark.benchmark_tools.generate_string_statistics import (
...
from data.helpers.download_datasets import download_dataset
...
def workload_gen(input):
    ...
    generate_workload(
        source_dataset, workload_path, max_no_joins=max_no_joins, **workload_args
    )
    return 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--workload_dir", default="data/workloads")

    parser.add_argument(
        "--database_conn",
        dest="database_conn_dict",
        action=StoreDictKeyPair,
        help="Database connection parameters in key=value format",
    ...
    parser.add_argument("--raw_dir", default="data/raw_plans")
    parser.add_argument("--parsed_plan_dir", default="data/parsed_plans")
    ...
    parser.add_argument(
        "--database",
        default=DatabaseSystem.POSTGRES,
        type=DatabaseSystem,
        choices=list(DatabaseSystem),
    )  # Currently only Postgres anyway
    ...
    if args.download_datasets:
        ...
        for rel_fit_dataset_name, dataset_name in tqdm(
            ...
            download_dataset(rel_fit_dataset_name, dataset_name, data_dir=args.data_dir)
    ...
    if args.scale_datasets:
        ...
        for dataset in database_list:
            ...
            assert (
                dataset.data_folder != dataset.source_dataset
            ), "For scaling a new folder is required"
    ...
    if args.print_run_commands:
        experiment_commands = []
        experiment_commands += gen_run_workload_commands(
            workload_name="workload_100k_s1", cap_workload=5000
        )
        # 5k queries with indexes
        experiment_commands += gen_run_workload_commands(
            workload_name="workload_100k_s2", cap_workload=5000, with_indexes=True
        )
        # 5k queries with complex predicates
        experiment_commands += gen_run_workload_commands(
            workload_name="complex_workload_200k_s1", cap_workload=5000
        )
    ...
    if args.parse_all_queries:
        ...
        print()
        print(
            f"Total plans parsed in {time.perf_counter() - start_t:.2f} secs: {eval.no_plans.sum()}"
        )
